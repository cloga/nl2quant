...
# 如何优化你的筛选策略？（Actionable Advice）

为了把这些“同质化垃圾信号”过滤掉，你需要给代码加几个过滤器：

1. 过滤同质化标的 (The "Same-Asset" Filter)
不要让追踪同一个指数的 ETF 互相对比。

方法： 获取 ETF 的跟踪指数代码。如果 Index_A == Index_B，直接 continue 跳过。

效果： 这样你就不会看到“华安黄金”配“国泰黄金”了。

2. 引入“波动率”门槛 (Volatility Filter)
你现在的配对价差（Spread）可能是一条死心电图。

方法： 计算价差序列的标准差 (std(Spread)).

规则： 如果 std(Spread) / price_mean < 0.01 (价差波动不到 1%)，说明没什么油水，剔除。

3. 强制跨品种/跨市场 (Cross-Matching)
人为引导算法去寻找更有趣的关系。

尝试 1： 现货 ETF vs 股票 ETF

例子： 黄金 ETF (518880) vs 黄金股 ETF (159562)

尝试 2： 产业链上下游

例子： 能源 ETF (159930) vs 化工 ETF (159870)

尝试 3： 强相关行业

例子： 家电 ETF (159996) vs 房地产 ETF (512200) （房卖得好，家电才能卖得好）

# 如果没找到配对的优化方向

这是一个非常棒的“负面反馈”结果！虽然看似失败（找到了0对），但这些日志其实暴露了**参数设置与模型假设之间的根本性冲突**。

不要急着怀疑市场没机会，这大概率是\*\*“漏斗口太窄”**或者**“刻度尺拿错了”\*\*。

我们需要从以下 4 个维度进行排查和参数优化：

-----

### 一、 核心矛盾：聚类用的是“收益率”，协整用的是“价格”

这是最容易导致“0 结果”的原因。

  * **聚类 (DBSCAN)：** 喂给它的是 **Daily Returns (日涨跌幅)**。它把“每天涨跌节奏一致”的 ETF 聚在了一起（比如 Cluster 5 的 36 只股票，大家节奏很像）。
  * **协整检验：** 喂给它的是 **Prices (收盘价)**。它要求两只 ETF 的**价格差**必须围绕均值波动。

**问题所在：**
一群 ETF 可以拥有极高的**相关性**（每天同涨同跌），但**价格差却越拉越大**（因为波动率不同，或者Beta不同）。

  * *例子：* 半导体 ETF (1.5倍波动) 和 科技 ETF (1.0倍波动)。它们每天方向一致（聚类成功），但半导体涨得快跌得也快，长期看两者价差像喇叭口一样张开（协整失败）。

**✅ 解决方案：**
你需要放宽协整检验的条件，或者先在聚类前对数据做处理。

  * **建议 1（最快）：** 暂时不要只看 P-value \< 0.05。先把阈值放宽到 **0.1** 甚至 **0.2**，看看是不是有一堆 0.06 的被你杀掉了。
  * **建议 2（根本）：** 在协整检验前，尝试对价格取对数 (`np.log(price)`)。很多时候 Log-Price 的协整性比 Raw Price 好得多。

-----

### 二、 参数调优：`eps` 和 `min_samples`
...
看你的日志：
  ...
  * `Cluster 5`: 36 只
  * `Cluster 28`: 3 只
  * 大部分 Cluster 只有 2-3 只。

**分析：**
`eps 0.5` 似乎把大部分 ETF 切得太碎了（很多只有2只），而 Cluster 5 那 36 只大概率是一个大板块（比如全是大盘蓝筹或全是TMT），内部虽然相关，但很难两两协整。

**✅ 优化建议：**

1.  **调大 `eps` (尝试 0.8 或 1.0)：** 让聚类的“容忍度”变高，把更多仅仅是“稍微有点像”的标的拉到一个池子里。池子大了，碰上协整的概率才大。
2.  **调大 `min_samples` (设为 3 或 5)：** 强制要求一个簇里至少有 3-5 只股票。这能过滤掉那些只有两只“孤儿”的无效聚类（通常那两只是偶然凑在一起的）。
...
**这是致命问题。**
如果你使用的是 ETF 的**不复权价格**（Raw Price），而这 365 天里 ETF 发生了**分红**或**拆分**，价格会突然跳水。

  * 协整模型会把这种“分红缺口”视为“价差极度偏离”，从而判定不协整。
  * 对于 ETF，分红比个股更频繁。

**✅ 检查清单：**

  * 确保你的数据源使用的是 **“后复权” (Adjusted Close)** 价格。
...
**逻辑：**
你用的是 365 天（一年）。在这一年里，如果正好赶上某行业（比如光伏或医药）单边暴跌，**均值回归**的特性会被强烈的**趋势项**掩盖。
协整检验（ADF Test）非常讨厌强趋势。如果两只 ETF 都在单边下跌，模型很难判断它们未来会不会回归。

**✅ 优化建议：**

  * **尝试 `--days 750` (约3年)：** 穿越牛熊的数据更容易找到长期稳定的协整关系。
  * **尝试 `--days 120` (半年) + 严格阈值：** 如果你想做短线套利，缩短时间，寻找短期内的强回归。

-----

### 🚀 **立即执行的行动计划**

不要改代码逻辑，先改参数运行 3 次，看看日志变化：

**测试 A：放宽聚类（扩大搜索池）**

```bash
--universe all_etf --days 750 --eps 0.8 --min_samples 3
```

  * *目的：* 用更长的数据（750天）和更松的聚类（0.8），看能不能捞出更多候选人。

**测试 B：放宽协整阈值（排查是否“差一点点”）**

  * *操作：* 修改代码中 `if pvalue < 0.05:` 改为 `if pvalue < 0.2:` 并且打印出 pvalue。
  * *目的：* 只要能看到 pvalue 是 0.08 或者 0.15，就说明方向对了，只是市场最近波动大。如果全出来的 pvalue 都是 0.8, 0.9，那就是数据错了。

**测试 C：更换板块（避开混乱的全市场）**

```bash
--universe industry_banking --days 365 --eps 0.5
```

  * *操作：* 如果你的代码支持行业筛选，先拿“银行 ETF”或“证券 ETF”这种公认协整性最好的板块试刀。如果连银行板块都跑不出结果，那就是**代码逻辑/数据源**有 Bug，而不是参数问题。

**总结：**
并不是没有套利机会，而是**DBSCAN 把形态相似的聚在一起，但这群相似的东西可能并没有数理上的“价差约束力”**。
先试着把 **时间拉长** (`--days 750`)，这是最容易见效的一招。