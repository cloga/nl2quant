...
# ============================================================================
st.set_page_config(
    page_title="DCA 策略对比 | NL-to-Quant",
    page_icon="📊",
    layout="wide",
)
...
st.title("🎯 DCA 策略对比分析")
st.markdown("""
在同一时间范围内对比多个定投策略的性能表现，找到最适合的投资方式。
""")

# ============================================================================
# Configuration
# ============================================================================
st.sidebar.title("⚙️ 对比配置")
...
# Run comparison
run_btn = st.button("🚀 运行对比分析", type="primary", use_container_width=True)
            ...
            try:
                ...
                for strategy_name, data in results_dict.items():
                    ...
                    comparison_data.append({
                        "策略": strategy_name,
                        "总投资额 (¥)": f"{metrics.get('total_invested', 0):,.0f}",
                        "期末资产 (¥)": f"{metrics.get('final_value', 0):,.0f}",
                        "总收益率": f"{metrics.get('total_return_pct', 0):.2f}%",
                        "CAGR": f"{metrics.get('cagr_pct', 0):.2f}%",
                        "年化波动": f"{metrics.get('volatility_pct', 0):.2f}%",
...
**三个策略的差异：**
- **普通定投**: 每期固定金额投资，适合稳定资金流的投资者
- **智能PE定投**: 根据市场估值动态调整投资金额，低估时多买，高估时少买
- **智能PB定投**: 根据市净率动态调整，适合关注资产质量的投资者
